...
RAGAS evaluation with all params and metrics logged to MLflow.

Usage:
    mlflow ui --port 5000 
    python -m src.evaluation.run_experiment --congid experiments/baseline.yaml
"""
...
def generate_answer(question: str, contexts: List[str], client , model:str) -> str:
  """
  Generate an answer grounded in retrieved context. Explicitly instructed to say when an answer isn't found.
  Retrieval can return irrelevant chunks, so generation must recognize and say so rather than hallucinate a confident answer.
  """
  context_text="\n\n---\n\n".join(contexts)
...
def run_experiment(config: dict):
  ...
  with mlflow.start_run(run_name=config.get("run_name","unnamed_run")):
    ...
    all_answers, all_contexts, latencies = [], [] , []

    for question in tqdm(EVAL_QUESTIONS, desc="Evaluation"):
      t0=time.time()
